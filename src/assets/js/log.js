...
function renderPedidosCocina() {
  ...
  pedidos.forEach((pedido, i) => {
    ...
    card.className = "order status-pending";
...
// Ventas por día (kitchenCompleted)
...
function enviarAlMesero() {
  ...
  // Aquí se puede agregar lógica adicional para enviar la orden al servidor o actualizar estados
...
async function iniciarSesion() {
  ...
  const usuarioEncontrado = usuarios.find(u => u.usuario.toLowerCase() === usuario && u.contrasena === contrasena);

  if (usuarioEncontrado) {
    alert(`¡Inicio de sesión exitoso como ${usuarioEncontrado.rol}!`);
    localStorage.setItem('usuario', usuarioEncontrado.rol);
    window.location.href = usuarioEncontrado.rol === 'admin' ? 'admin.html' : 'mesero.html';
    return;
  }

  // Intentar login remoto vía API
  try {
